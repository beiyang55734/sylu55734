...
@app.put("/api/voices/{voice_file}")
async def upload_voice_file(voice_file: str, request: Request):
    try:
        if "/" in voice_file or "\\" in voice_file:
        ...
        with open(file_path, "wb") as f:
            f.write(data)
        return JSONResponse(
            content={"message": "Upload successful", "url": f"/assets/voices/{voice_file}"},
            status_code=200
        )
