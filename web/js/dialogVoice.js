...
document.addEventListener('DOMContentLoaded', function() {
    ...
    voiceInputText = voiceInputArea ? voiceInputArea.querySelector('span') : null;
    ...
    // 处理聊天历史显示状态
    if (!chatHistoryShowed && chatContainer) {
...
async function tts_realtime_ws(voice_id, model_name) {
    try {
        ...
        console.warn('语音服务连接失败，继续执行文字对话:', error);
        // 不显示错误提示，避免影响用户体验
        // 即使语音服务失败，也继续执行文字对话
    }
}
...
async function sendTextMessage(inputValue) {
    ...
    try {
        if (window.parent.embeddingManager.memories) {
            // 尝试获取记忆，但不依赖语音服务凭证
            try {
                const queryEmbedding = await window.parent.embeddingManager.getEmbedding(inputValue, "");
                ...
                console.warn('获取记忆嵌入失败，继续执行文字对话:', embeddingError);
                similarMemoryTextList = [];
            }
        }
    } catch (error) {
        console.warn('获取记忆失败，继续执行文字对话:', error);
        similarMemoryTextList = [];
    }
    ...
    if (inputValue) {
        try {
            ...
            try {
                ...
                console.warn('语音服务初始化失败，继续执行文字对话:', ttsError);
                // 即使语音服务失败，也继续执行文字对话
            }
...
function updateInterruptButton() {
    if (interruptButton) {
        ...
        if (!hasPlayedOnce) {
            ...
            interruptButton.title = '启动';
        } else {
            iconEl.textContent = 'pause';
            interruptButton.title = '停止';
        }
        // 移除静音检查，因为静音按钮已隐藏
