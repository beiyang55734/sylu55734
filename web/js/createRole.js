...
async function handleBack() {
    if (isUploading) {
        XSAlert('视频上传中，请等待完成后再操作');
        return;
    }
    const confirmed = await XSConfirm('确定要放弃当前编辑内容吗？');
    if (confirmed) {
        history.back();
    }
}
...
async function handleCreate() {
    ...
    const confirmed = await XSConfirm('请保证图片或视频符合要求，本次创建成功将消耗1次机会，创建失败将消耗0.2次机会');
    if (!confirmed) {
        return;
    }
    ...
    // 访问服务器确认还有多少创建机会
    try {
        const response = await fetch(url_prefix + '/auth/apply_new_role', {
            method: 'POST',
            headers: {
